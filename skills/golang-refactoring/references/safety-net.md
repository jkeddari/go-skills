...
→ See `golang-architecture` skill for constructor and dependency-injection patterns this move builds on, and [catalog.md](catalog.md) in this skill for the Sprout/Wrap mechanics that typically pair with a freshly introduced seam.
...
- → See `golang-performance` skill for interpreting a `benchstat` delta and the full profiling methodology.
