...
- The same one-row-one-concern discipline holds even within a single letter:
  ...
  - a move is verified by gopls plus a green build/test run, while an optimization needs benchmarks (→ See `golang-performance` skill) and a closer read for subtle correctness changes
...
3. A human reviews and merges each of these small PRs into `refactor/<topic>` at their own pace.
   ...
   - For any PR that changes code logic rather than just its shape, load `golang-security` (and `golang-code-quality` for internal-correctness risk) alongside this skill before approving it, since a logic change can introduce a vulnerability or a bug that a purely mechanical refactor never could.
...
- [go-tooling.md](go-tooling.md) — gopls operations, CLI invocation, `gofmt -r`, and `go/analysis` fixers referenced throughout the inventory examples above.
...
- → See `golang-security` skill (and `golang-code-quality`) for reviewing any PR that changes code logic, per Step 3 above.
