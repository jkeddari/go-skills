...
→ See `golang-code-quality` skill for what to rename identifiers _to_. This entry covers only _how_ to apply the rename safely at scale.
...
→ See `golang-architecture` skill for converting a long parameter list into an options struct rather than just reordering or trimming it.
...
→ See `golang-architecture` skill for target package layout. See [structural.md](structural.md) for the full type-alias gradual-repair recipe.
...
→ See `golang-architecture` skill for target package layout.
...
→ See `golang-code-quality` skill for the full early-return style rule this refactor works toward.
...
→ See `golang-architecture` skill for functional options as the alternative when the struct exists to configure construction rather than to group a plain data clump.
...
→ See `golang-architecture` skill for the target interface-dispatch pattern.
...
- → See [go-tooling.md](go-tooling.md) for gopls operations, `gofmt -r`, and official analysis-package guidance.
- → See `golang-code-quality` skill for what to rename identifiers _to_.
- → See `golang-architecture` skill for target package/directory layout.
- → See `golang-architecture` skill for options structs, consumer-side interfaces, and interface-dispatch patterns referenced throughout this catalog.
- → See `golang-code-quality` skill for the guard-clause/early-return style rule.
