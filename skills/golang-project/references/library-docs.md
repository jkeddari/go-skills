...
Create runnable examples manually at <https://go.dev/play/> when a playground link materially helps readers.
...
Add a `llms.txt` file at the repository root when it helps agents understand the project. Copy the [llms.txt template](../assets/docs/llms.txt) and remove sections that do not apply.
