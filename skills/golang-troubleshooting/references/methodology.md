...
# Run linters — see the golang-tooling skill for configuration
golangci-lint run ./...
```

Run `golangci-lint` early in your debugging workflow. It catches unchecked errors, suspicious constructs, and many other issues that are easy to miss by reading code. See the `golang-tooling` skill for configuration and usage.
...
## Step 5: Check Existing Observability

Production debugging should begin with signals already collected by the project. Determine which logs, metrics, traces, profiles, deployment events, and infrastructure signals are available before adding instrumentation.

Use them to answer concrete questions:

- Did error rate or latency change at the same time as a deployment?
- Is the symptom isolated to a version, instance, region, route, or dependency?
- Are CPU, memory, goroutines, queues, connection pools, or disk saturated?
- Does a trace identify the slow or failing boundary?
- Does the first error differ from the later cascade visible to users?

If the necessary signal is missing, add the smallest temporary diagnostic that can discriminate the current hypothesis. Protect sensitive telemetry, record its overhead, and remove or disable it after the investigation.
...
- **Each fix reveals a new problem:** You're chasing symptoms, not the root cause. Return to the workflow in [SKILL.md](../SKILL.md).
