...
Use `runtime.NumGoroutine()` for coarse monitoring, `/debug/pprof/goroutine?debug=2` for stack dumps, and `go test -race ./...` for race checks. A count alone does not prove a leak; compare repeated snapshots and identify stacks whose population grows without returning to baseline.
...
// In tests, expose an explicit shutdown path and wait for owned workers.
// A bounded wait makes a leaked worker fail the test without a helper library.
```
