...
For large systems, each context can be its own Go module in a workspace (`go.work`). See the `golang-architecture` skill for workspace setup.

## Wiring

Compose dependencies explicitly at the executable boundary with constructors. → See `golang-architecture` for manual wiring and lifecycle guidance.
