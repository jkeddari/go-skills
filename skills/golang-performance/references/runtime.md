...
**Diagnose:** 1- `GODEBUG=gctrace=1` — print one line per GC cycle; look for high GC frequency, CPU cost, or a heap growing faster than expected 2- `runtime.ReadMemStats` — inspect `Alloc`, `TotalAlloc`, `NumGC`, and `PauseNs` 3- `go tool trace` — visualize stop-the-world pauses and GC assist 4- `runtime/metrics` — monitor GC cycle, heap allocation, and pause distributions over equal time windows 5- `GODEBUG=gcpacertrace=1` — trace pacer decisions when simpler evidence is insufficient
...
**Diagnose:** 1- `go tool pprof -alloc_objects` — rank functions by allocation count 2- `GODEBUG=gctrace=1` — compare GC frequency before and after the change 3- `runtime/metrics` — compare allocation rates over equal production windows
...
- **Preallocate slices and maps** — → See `golang-code-quality` skill
- **Avoid interface boxing** in hot paths — use typed parameters or generics

## GOMAXPROCS in Containers

**Diagnose:** 1- `go tool pprof` — look for high `runtime.schedule` or `runtime.findRunnable` overhead 2- `go tool trace` — check distribution across P's 3- `GODEBUG=schedtrace=1000` — inspect run queues and idle P's 4- `runtime.GOMAXPROCS(0)` — compare the current value with the deployment CPU limit 5- existing process CPU telemetry — determine whether the process is actually CPU-saturated
...
In a container with 2 CPU cores on a 64-core host running Go 1.25+ with **cgroup v2**, `GOMAXPROCS` should resolve to 2 by default. Verify the actual value in the target environment instead of assuming runtime and cgroup behavior. On older Go versions or cgroup layouts that are not detected correctly, set `GOMAXPROCS` explicitly in deployment configuration.
