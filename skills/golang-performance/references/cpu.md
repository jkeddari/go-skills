...
**Diagnose:** 1- `go tool pprof` (goroutine profile) — look for many goroutines stuck in `"runnable"` while one monopolizes execution 2- `go tool trace` — inspect scheduling gaps 3- `GODEBUG=schedtrace=1000` — inspect run queues across P's 4- `runtime/metrics` (`/sched/latencies:seconds`) — measure scheduler latency 5- existing process CPU telemetry — confirm saturation against `GOMAXPROCS`
