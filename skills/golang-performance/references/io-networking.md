...
**Diagnose:** 1- `go tool pprof` (goroutine and block profiles) — look for goroutines blocked on transport dialing or reads 2- `go tool trace` — inspect wall-clock gaps caused by network I/O 3- existing goroutine telemetry — check whether counts rise under stable load 4- focused request timing — separate DNS, connect, TLS, server, and body-read time
...
Do not replace the serializer solely from a generic benchmark. Compatibility, number handling, duplicate keys, invalid UTF-8, escaping, and custom marshal behavior are part of the contract and must be tested against the project's payloads.
