...
→ See `golang-concurrency` for duplicate-work suppression and the `sync.Map` versus `RWMutex` decision.

### Bounded caches

A bounded cache needs an explicit size unit, eviction policy, concurrency policy, and invalidation contract. `map` plus `container/list` can implement LRU behavior with the standard library, but measure allocation and lock costs before assuming it suits a hot path. Cache correctness and bounded growth matter more than the eviction algorithm's microbenchmark.
